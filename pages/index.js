...
export default function Home() {
  ...
  function markExcel(modifiedPdfs) {
    if (workbook.current && modifiedPdfs?.length > 0) {
      ...
      if (withoutCells !== "") {
        alert(
          `The following invoices did not have a reference in the excel file: ${withoutCells}`
        );
      }
  ...
  async function handleDownload() {
    ...
    if (pdfFiles.current) {
      await Promise.all(
        pdfFiles.current.map(async (currentPdf, index) => {
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <div className="form">
          ...
          {showDownload && (
            <div>
              <button onClick={handleDownload} className="magic-button dl">
                Download Zip
              </button>
            </div>
          )}
